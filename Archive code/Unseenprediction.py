...
# Load the dataset, Z_dim is len(your data) + latent Vector. For example 200 + 50
dataset = MMIUnseenDataset(
                     z_dim=250,
                     points_path=r'',
                     )

# Output the result path & load the data into Generator
results_folder = r''
...
for p in dataset:
    ...
    cv2.imwrite(results_folder + '\\' + '_' + str(i+1) + '.png', img)
    i += 1
